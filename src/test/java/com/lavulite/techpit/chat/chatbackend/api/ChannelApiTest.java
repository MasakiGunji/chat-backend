...
public class ChannelApiTest {
  ...
  public void createTest(String requestBody, String expectedBody, String dbPath) throws Exception {
    ...
    mockMvc.perform(
      MockMvcRequestBuilders.post("/channels") // postメソッドを利用
    ...
    System.out.println(expectedChannelsTable);
    System.out.println(actualChannelsTable);
  ...
  public void findAllTest(String expectedBody, String dbPath) throws Exception {
    ...
    mockMvc.perform(
    MockMvcRequestBuilders.get("/channels") // getメソッドを利用
        .contentType(MediaType.APPLICATION_JSON)
        .accept(MediaType.APPLICATION_JSON_UTF8))
    .andExpect(MockMvcResultMatchers.status().isOk())
    .andExpect((result) -> JSONAssert.assertEquals(
  ...
  public void updateTest(int id, String requestBody, String dbPath) throws Exception {
    ...
    mockMvc.perform(
        MockMvcRequestBuilders.put("/channels/" + id) // URLでidを指定
            .content(requestBody)
            .contentType(MediaType.APPLICATION_JSON)
            .accept(MediaType.APPLICATION_JSON_UTF8))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect((result) -> JSONAssert.assertEquals(
            expectedBody, // 作成した想定結果と一致することを検証
            result.getResponse().getContentAsString(),
            false));
